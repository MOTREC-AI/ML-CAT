tester <- function(train_kf, train_kf_y, val_kf, val_kf_y, ks_train_TrueFalse, ks_val_TrueFalse,
                   algo, mtry, nodesize, ntrees)
{
  ...
  ks_train_TrueFalse <- as.data.frame(fread(ks_train_TrueFalse))
  ks_val_TrueFalse <- as.data.frame(fread(ks_val_TrueFalse))

  # Get the file with the _ks covariate data
  x <- train_kf
  x <- str_replace(x, regex(".*/"), "")
  covariateSet <- str_replace(x, regex("_train_features.*"), "")

  all_ks_data <- as.data.frame(fread(paste0("/data/aiml/cadleyj/vte-data/rohanNonstandardData20220428/", covariateSet, 
                                            "_train_features_full_ks_imputed_iterative.csv")))

  # Fix the # problem
  colOnename <- colnames(all_ks_data)[1]
  colOnename <- str_replace(colOnename, "# ", "")
  colnames(all_ks_data)[1] <- colOnename

  ######################################################################################################
  # Kludge to get rid of CMP features when required for a covariate set. Comment out otherwise.
  ...
  ######################################################################################################
  # Kluge to do the "Basic Genetic" covariate set. It's "No Labs", with the following adjustment (comment out otherwise):
  ...
  # See below as well for another Basic Genetic kludge.
  ######################################################################################################
  ...
  # ks data edits here...
  ks_cols <- grepl("_ks", colnames(all_ks_data))          # Find the_ks columns
  ks_cols_NoKS <- colnames(all_ks_data)[ks_cols]          # Put the ks names in a vector
  ks_cols_NoKS <- str_replace(ks_cols_NoKS, "_ks", "")    # Strip the _ks from the col names

  all_ks_data <- all_ks_data[, ks_cols]                   # Keep the data frpm the _ks cols only.
  all_ks_data <- all_ks_data[ks_val_TrueFalse$ks_index,]  # Get only the data for this fold, in the correct order

  val_data[,ks_cols_NoKS] <- all_ks_data
  val_data <- val_data[ks_val_TrueFalse$KS,]
  val_yVals <- val_yVals[ks_val_TrueFalse$KS,]

  ######################################################################################################
  # More kludging for Basic Genetic...
...
batcher_kf <- function(train, train_y, val, val_y, ks_train_TrueFalse, ks_val_TrueFalse,
                       algo, dataDir, outPrefix, LSB_JOBINDEX, mtry, nodesize, ntrees)
{
  ...
  ks_train_TrueFalse <- paste0(dataDir, ks_train_TrueFalse, "_", i, ".csv")
  ks_val_TrueFalse <- paste0(dataDir, ks_val_TrueFalse, "_", i, ".csv")

  output_kf <- tester(train_kf, train_kf_y, val_kf, val_kf_y, ks_train_TrueFalse, ks_val_TrueFalse,
                      algo, mtry, nodesize, ntrees)
...
comparator_py <- function(train, train_y, val, val_y, ks_train_TrueFalse, ks_val_TrueFalse,
                          algo, dataDir, outPrefix, LSB_JOBINDEX, mtry, nodesize, ntrees)
{
  print("in comparator")

  batcher_kf(train, train_y, val, val_y, ks_train_TrueFalse, ks_val_TrueFalse,
             algo, dataDir, outPrefix, LSB_JOBINDEX, mtry, nodesize, ntrees)
}
