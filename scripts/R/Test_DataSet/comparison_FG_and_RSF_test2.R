...
library(randomForestSRC)  # v2.12.1
# library(randomForestSRC, lib.loc = <PATH to lib>)  # v3.1.0
...
val <- args[5]
val_y <- args[6]
mtry <- as.integer(args[7])
nodesize <- as.integer(args[8])
ntrees <- as.integer(args[9])
...
source("comparison_utility_functions_test2.R")
