...
colnames(all_event_data2) <- c("EVENT_6", "OBS_TIME_6") # We'll populate these in the next lines.
...
predictor <- function(model, data, covars, algo)
{
  ...
  if (algo == "rsf") {
    ...
    # save events and cifs for the calibration plot
    data_180 <- data$EVENT_6
    predictions_180 <- predictions.180
    save(predictions_180, data_180, file="Out/calibration_data.rda")
